...
//重力加速度
static const CGFloat kGravityAcceleration = 30.0f;
...
- (UIImage*)createRandomImage{
	...
	NSString *imageName = [NSString stringWithFormat:@"snow%d", imageIndex];
...
- (void)updatePos{
	//消失：出了底边界，则重设信息
	//todo:添加处理其它边界同样处理
	if (self.center.y  > maxCenterPos.y
		|| self.center.y  < 0.0
		|| self.center.x  > maxCenterPos.x
		|| self.center.x  < 0.0) {

...
@interface SnowWeatherView(private)
...
#define SNOW_BG_NAME @"snowy_bg.jpg"
...
- (id)initWithOrientation:(UIInterfaceOrientation)orientation{
    if ((self = [super init])) {
		...
		//设置背景
		UIImage *image = [UIImage imageNamed:SNOW_BG_NAME];
		backgroundImageView = [[UIImageView alloc] initWithImage:image];		
		[self addSubview:backgroundImageView];
		//背景移动动画
		[self bgMoveAnimation];
		
		particleViews = [[NSMutableArray alloc] initWithCapacity:SNOW_COUNT];
		//横竖转换时，设置最大下落位置
		[SnowView setScreenSizeWithOrientation:screenOrientation];
		[SnowView setTimerInterval:PARTICAL_TIMER_INTERVAL];	
		
		for (int i = 0; i < SNOW_COUNT; ++i) {
			SnowView* snowView = [[SnowView alloc] init];
			[self addSubview:snowView];
			[particleViews addObject:snowView];
			[snowView release];
		}
...
- (void)resetWithOrientation:(UIInterfaceOrientation)orientation{	
	...
	[SnowView setScreenSizeWithOrientation:screenOrientation];
	//重设雪花信息	
	for (SnowView* snowView in particleViews) {
		[snowView reset];
	}
...
- (void)updateViewsPos{
	//如果所有视图没有都生成，随机生成几个
	for (SnowView *view in particleViews) {
		[view updatePos];
	}
}
- (void)bgMoveAnimation{

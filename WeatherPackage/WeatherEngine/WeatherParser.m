...
- (id)init{
	...
	if (self) {
        ...
        ipCityLocation = [[IPCityLocation alloc] init];
		[self initXMLParser];
	}
	return self;
}
...
- (void)splitXmlWeatherInfo{
	...
	if ([xmlWeatherStringArray count] >= 30) {
        ...
        dayWeather.city = [ipCityLocation citySimpleName];
...
- (void)outputDayWeatherInfo{
	// out put
	for (DayWeather *dayWeather in dayWeatherArray) {
		NSLog(@"=========================");
		NSLog(@"%@", [dayWeather dateStr]);
		NSLog(@"%@", [dayWeather weekStr]);		
		NSLog(@"%@", [dayWeather temperatureRangeInfo]);
		NSLog(@"%@", [dayWeather windInfo]);
		NSLog(@"%@", [dayWeather basicInfo]);
		NSLog(@"%@", [dayWeather picureStartName]);
		NSLog(@"%@", [dayWeather picureEndName]);
	}
}
/*
 *蔡勒公式
